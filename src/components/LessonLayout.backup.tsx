...
// wallet stuff - imported directly since LessonLayoutInner needs useAccounts
...
function LessonLayoutInner({ lesson, initialChapter: propChapter, initialStep: propStep }: LessonLayoutProps) {
  ...
  // Pending generation params (stored when wallet was required)
  ...
  // Helper to get connected wallet address
  const getWalletAddress = useCallback(() => {
    return accounts?.[0]?.address || undefined;
  }, [accounts]);

  // Trigger generation with wallet address
  const triggerWithWallet = useCallback((
    ...
    // Store pending generation in case wallet is needed
  ...
  // Handle wallet connected - retry generation with wallet
          ...
          <div className={`relative overflow-hidden transition-all duration-500 p-5 ${
            ...
            <div className="w-full h-full rounded-xl border border-purple-500/30 bg-slate-900 flex flex-col overflow-hidden">
              ...
              <div className="flex justify-between items-start p-4 flex-shrink-0">
                <Link href="/lab" className="flex items-center space-x-2 hover:opacity-80 transition-opacity" title="View all lessons">
                  <img
                    src="/logo.png"
                    alt="Monsters ink!"
                    className="h-[80px]"
...
// WithSigner moved to WalletProviders.tsx for lazy loading
